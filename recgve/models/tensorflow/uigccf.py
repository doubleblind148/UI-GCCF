...
class UIGCCF(keras.Model, RepresentationsBasedRecommender):
    ...
    def __init__(
        ...
        proj_item_adjacency = self._build_item_graph_adjacency(urm, top_k)
        self.S = to_tf_sparse_tensor(proj_item_adjacency)

    @timing
    def _build_item_graph_adjacency(self, urm, top_k):
        adj = cosine_similarity(urm.T, urm.T, dense_output=False)
        # XXX maybe add self loop?
        S = truncate_top_k_2(adj.todense(), top_k)
        S = sps.csr_matrix(S)
        S = normalize_csr_sparse_matrix(S)
        return S
    ...
    def __call__(self, urm, training=True):
        ...
        if training:
            S = tf_dropout_sparse(
                self.S, 1 - self.edge_dropout, self.S.values.get_shape()[0]
            )
