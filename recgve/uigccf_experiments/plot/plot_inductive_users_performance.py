...
if __name__ == "__main__":
    ...
    for p in PROJECTS:
        ...
        score = []
        for r in runs:
            if BASE in r.name and "val" not in r.name:
        ...
        res_df = pd.DataFrame(
            zip(kind, map(float, seen_users_percentage), metric, score),
            columns=["kind", "seen_users_percentage", "metric", "score"],
        )
        ...
        for c in CUTOFFS:
            ...
            ax.set_xlabel("Train user percentage")
            ax.legend().set_title("")
            ...
            # ax.legend(
            #     handles=[handles[x] for x in [1, 2, 4, 5]],
            #     labels=[labels[x] for x in [1, 2, 4, 5]],
            # )
