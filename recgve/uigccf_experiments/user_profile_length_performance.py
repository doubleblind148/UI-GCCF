...
from uigccf_experiments.best_models import get_wandb_project_dict, restore_models
import numpy as np
import os
...
PROJECT = "gowalla_dat"
MODELS = ["igcmf", "lightgcn", "bprmf", "ngcf"]
METRICS = ["Recall", "NDCG"]
CUTOFFS = [5, 20]
N = 4
        ...
        for c in CUTOFFS:
            ...
            if PROJECT != "lastfm_dat":
                ...
                for t in ax.legend_.texts:
                    ...
                    elif t.get_text() == "IGCMF":
                        t.set_text("UI-GCCF")
                ax.set_ylabel(f"{m}@{c}")

            ax.tick_params(direction="in")
            ax.tick_params(direction="in")
            ax1.set_ylabel("Run time s/1e3") if PROJECT == "gowalla_dat" else plt.gca().set_ylabel("")
